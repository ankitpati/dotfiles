...
( GH_USERNAME='ankitpati'; age -r "$(curl --header "Authorization: token $(lpass show --password github_personal_access_token)" "https://github.example.org/api/v3/users/$GH_USERNAME/keys" | jq -r .[0].key)" -o cipher.txt.age plain.txt )
...
docker run -it -e 'TERM=xterm-256color' --rm image_name
